...
export default function ApprovalsCommandCenter() {
  ...
  return <div className="space-y-4" data-testid="approvals-command-center">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(280px,.72fr)_minmax(0,1.5fr)]">
      ...
      <section className="min-h-[440px] border bg-card p-4">{!selected ? <EmptyState className="min-h-[400px]" title={tr("اختر اعتمادًا", "Select an approval")} description={tr("اعرض ملخص القرار والمستندات الداعمة هنا.", "Its decision summary and supporting documents will appear here.")} /> : <div className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-bold" dir="ltr">{selected.approval_number}</h3><p className="text-sm text-muted-foreground">{selected.project_name || tr("بدون مشروع", "No project")} · {tr(`الإصدار ${selected.revision_number + 1}`, `Revision ${selected.revision_number + 1}`)}</p></div><StatusBadge tone={statusTones[selected.status] || "neutral"}>{dictionaryLabel(statusLabels, selected.status, tr, selected.status)}</StatusBadge></div>
