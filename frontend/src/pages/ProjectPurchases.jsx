...
export default function ProjectPurchases() {
  ...
  return <div className="space-y-4" dir={direction}>
    ...
    <section className="grid grid-cols-2 gap-2 lg:grid-cols-3 xl:grid-cols-6" data-testid="project-formal-kpis">
      ...
      <KpiCard icon={WalletCards} label={tr("قيمة أوامر الشراء", "PO Value")} value={projectMoney(k.formal_po_total || 0)} />
      <KpiCard icon={CreditCard} label={tr("المدفوع", "Paid")} value={projectMoney(k.formal_paid_amount || 0)} tone="success" />
      <KpiCard icon={CreditCard} label={tr("المتبقي", "Outstanding")} value={projectMoney(k.formal_outstanding_amount ?? k.formal_po_total ?? 0)} tone="warning" />
    ...
    <Tabs defaultValue="overview" dir={direction} className="space-y-3">
      ...
      <TabPanel value="approvals"><RecordCards icon={FileCheck2} items={hub.approvals || []} empty={tr("لا توجد اعتمادات مرتبطة.", "No linked approvals.")} title={(x) => x.approval_number} subtitle={(x) => <>{x.engineer_name || tr("اعتماد داخلي", "Internal approval")} · {projectMoney(x.final_total)}</>} status={(x) => x.status} onOpen={(x) => navigate("/approvals", { state: { approval_id: x.id } })} labels={labels} /></TabPanel>
      <TabPanel value="purchase_orders"><RecordCards icon={ShoppingCart} items={hub.purchase_orders || []} empty={tr("لا توجد أوامر شراء مرتبطة.", "No linked purchase orders.")} title={(x) => x.po_number} subtitle={(x) => <>{x.supplier_name || "-"} · {projectMoney(x.final_total)}</>} status={(x) => x.status} onOpen={(x) => navigate(`/purchase-orders/${x.id}`)} labels={labels} /></TabPanel>
      <TabPanel value="payments"><RecordCards icon={CreditCard} items={formalPayments} empty={tr("لا توجد دفعات رسمية مسجلة لهذا المشروع.", "No formal PO payments are recorded for this project.")} title={(x) => x.payment_number} subtitle={(x) => <>{new Date(x.payment_date).toLocaleDateString(locale)} · {projectMoney(x.amount)}</>} onOpen={(x) => navigate(`/purchase-orders/${x.purchase_order_id}`)} labels={labels} /></TabPanel>
