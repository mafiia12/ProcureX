  ...
  return (
    <div className="space-y-5" data-testid="purchase-orders-page">
      ...
      <FilterBar resultLabel={tr(`${filteredOrders.length} من ${orders.length} أمر شراء`, `${filteredOrders.length} of ${orders.length} purchase orders`)} onClear={clearFilters} data-testid="purchase-order-filters">
        ...
        <select value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)} className="h-8 rounded-md border bg-background px-2.5 text-sm"><option value="">{tr("كل المشاريع", "All projects")}</option>{projects.map((value) => <option key={value}>{value}</option>)}</select>
        <select value={supplierFilter} onChange={(event) => setSupplierFilter(event.target.value)} className="h-8 rounded-md border bg-background px-2.5 text-sm"><option value="">{tr("كل الموردين", "All suppliers")}</option>{suppliers.map((value) => <option key={value}>{value}</option>)}</select>
        <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="h-8 rounded-md border bg-background px-2.5 text-sm" data-testid="purchase-order-status-filter"><option value="">{tr("كل حالات PO", "All PO statuses")}</option>{Object.entries(PO_STATUS_LABEL).map(([value, label]) => <option key={value} value={value}>{tr(label, { draft: "Draft", approved: "Approved", sent: "Sent", supplier_confirmed: "Supplier confirmed", in_delivery: "In delivery", partial_received: "Partial receipt", delivery_problem: "Delivery problem", completed: "Completed", cancelled: "Cancelled" }[value])}</option>)}</select>
